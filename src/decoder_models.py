...
class MixtralModelForProbing(torch.nn.Module):
    
    def __init__(self, tokenizer: AutoTokenizer):
        
        super(MixtralModelForProbing, self).__init__()
        self.pretrain_mixtral_name = tokenizer.name_or_path
        bnb_config = BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=torch.bfloat16)
        self.model = AutoModelForCausalLM.from_pretrained(self.pretrain_mixtral_name, quantization_config=bnb_config, device_map="auto")
        self.tokenizer = tokenizer
    
    def hook_function(self, module: torch.nn.Module, inputs: torch.tensor, outputs: torch.tensor):
        self.ff_out_neuron_output_list.append(outputs[0].detach())
    ...
    def register_hook(self):
        ...
        self.ff_out_neuron_output_list = [] # List[L x (b, T, d)]
        for L in self.model.model.layers:
            h = L.block_sparse_moe.register_forward_hook(self.hook_function)
            self.hooks_list.append(h)
...
def inference(model_name: str, is_mlama: bool) -> None:
    ...
    if "llama" in model_name.lower():
        ...
        model = MixtralModelForProbing(tokenizer=tokenizer).to("cuda")
...
if __name__ == "__main__":
    ...
    inference(models[1], is_mlama=False)
